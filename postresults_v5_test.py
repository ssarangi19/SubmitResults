...
1) Change program types that receive supplements (Ixcela FX Program, Ixcela Pro Complete, Ixcela Pro Supported,
Ixcela Complete)

2) Add notification to Partner/Pro that client report is ready

3) Add notification to Sales that Partner report is ready

10/24/19
Edits:

1) Check address from MODX before generating supplement file, and update in LIMS

01/21/20
Edits:

1) Added 'LastSupDate' UDF (date supplements are being sent for this kit if applicable)
2) Added 'ResultPostedDate' UDF (date results are being posted)
...
"""
headersModx = {'Authorization': 'Bearer fcefefa3d854ce12a88983ce6070c4056baab98f',
			   'Content-Type': 'application/json'}
"""
...
# Hubspot email to Partner/pro
def hubspot_partner(tId, cust_email, fname, clientName, headers):

	pURL = 'https://api.hubapi.com/email/public/v1/singleEmail/send'

	payload = {'emailId': tId, 'message': {'to': cust_email},
			   'contactProperties': [{"name": "firstname", "value": fname}, {"name": "client_name", "value": clientName}]}
...
# Check MODX for Address

def checkadd(currwebID, planType):

	usrUrl = 'http://dev2.ixcela.modxcloud.com/api/v1/users.json'  # endpoint to export user info
	#usrUrl = 'https://ixcela.com/api/v1/users.json'  # endpoint to export user info

	tempDict = dict()
	tempDict['data'] = dict()

	tempDict['data']['user'] = int(currwebID)
	...
	try:
		...
		temp_address = str((usrData['meta']['shipping_company']).encode('ascii', 'ignore')) + '<abr>' + str(
			(usrData['meta']['shipping_address1']).encode('ascii', 'ignore')) + '<abr>' + str(
			(usrData['meta']['shipping_address2']).encode('ascii', 'ignore')) + \
					   '<abr>' + str((usrData['meta']['shipping_city']).encode('ascii', 'ignore')) + '<abr>' + str(
			(usrData['meta']['shipping_state']).encode('ascii', 'ignore')) + '<abr>' + \
					   str((usrData['meta']['shipping_postal_code']).encode('ascii', 'ignore')) + '<abr>' + str(
			(usrData['meta']['shipping_country']).encode('ascii', 'ignore'))
		...
		if planType == 'Prepaid':
			...
			temp_address = '' + '<abr>' + str(
				(usrData['attributes']['address']).encode('ascii', 'ignore')) + '<abr>' + '' + '<abr>' + str(
				(usrData['attributes']['city']).encode('ascii', 'ignore')) + \
						   '<abr>' + str((usrData['attributes']['state']).encode('ascii', 'ignore')) + '<abr>' + str(
				(usrData['attributes']['zip']).encode('ascii', 'ignore')) + \
						   '<abr>' + str((usrData['attributes']['country']).encode('ascii', 'ignore'))
...
# Function to check 'Next Step' UDF, 'Post' results to MODX, create a supplement file to upload to Makers,
# and call 'setnxtsteps'
...
def post2user():
	...
	# serving for each supplement (4 mo supply)
	suppbottles = {'1': 2, '2': 4, '3': 1, '4': 2, '5': 2, '6': 2, '7': 2, '8': 2}
	...
	# no supplements for 'Ixcela Pro Directed'

	# plans for supplements

	compProLst = ['Ixcela Pro Complete', 'Ixcela Pro Supported', 'Ixcela FX Program', 'Ixcela Pro Maintain']

	# Subscription Names (over time) for Ixcela Complete

	compNames = ['Ixcela Complete', 'Complete', 'Monthly Subscription', 'Ixcela Unlimited']  # plans for supplements

	# Single Kit Products - used for the 'Upgrade to Subscription' option

	kitNames = ['Ixcela Assess', 'Assess', 'Assess (BP)', 'Prepaid', 'Promo', 'Ixcela Pro Assess', 'Ixcela FX Snapshot']

	# Ixcela FX Program gets the ability to purchase another retest kit but the upgrade endpoint has to be called

	otherUpgrades = ['Ixcela FX Program']
	...
	for auri in art_uris2:
		...
		if str(nstepudf[0].firstChild.data) == 'Stop':
			...
			# Partner/ Pro Related
			proCode = str(api.getUDF(sDOM, 'procode'))  # '' or 'partner' or '<procode>'
			partDetails = str(api.getUDF(sDOM, 'partner_email'))  # 'profirstname, proemail' or ''
			...
			#urlModx = 'https://ixcela.com/api/v1/save-results.json'
			...
			if rModx.status_code != requests.codes.ok:  # did not POST successfully
				...
				# Update 'ResultPostedDate' UDF to today's date
				resDate = api.getUDF(sDOM, 'ResultPostedDate')
				resDate = today_date
				api.setUDF(sDOM, 'ResultPostedDate', resDate)
				...
				if sCode == 'Sales':
					salesDict[samp_name] = [currfname, currlname, 1]  # 1 indicates regular sales

				if proCode == 'partner':  # Partner/ Pro's results have been posted
					salesDict[samp_name] = [currfname, currlname, 99]  # 99 indicates Pro/Partner

				# ******************************************************************* #
				# Notify client's Pro

				if partDetails != '':  # an associated pro exists

					partinfoLst = partDetails.split(',')
					profname = partinfoLst[0].strip()  # first name of partner/pro
					proEmail = partinfoLst[1].strip()  # email of partner/pro

					client_name = currfname + ' ' + currlname
					tId = 14017520172

					hubspot_partner(tId, proEmail, profname, client_name, headers)
				# ******************************************************************* #
				...
				if proCode == 'partner':
					resId = 7596572684  # D2C or 'Partner' or 'Prepaid'

				elif proCode == '':  # will be empty for a D2C customer
					resId = 7596572684  # D2C or 'Partner' or 'Prepaid'

				elif proCode == 'ambrosia' or sCode == 'ambrosia':
					resId = 21609689149

				else:
					resId = 14013834936  # Pro Client

				hubspot_trans(resId, curremail, currfname, selplanType, headers)

				# ******************************************************************* #

				# Append customer to list to 'Upgrade to Complete' if selplanType is in kitNames

				if sCode != 'ambrosia' or proCode != 'ambrosia':
					if (selplanType in kitNames) or (selplanType in otherUpgrades):
				...
				if (selplanType in compProLst) or (selplanType in compNames):
					...
					# TO DO #
					# Check MODX for updated address and update LIMS

					curradd = checkadd(currwebID, selplanType)

					# Update Address

					tempVal = api.getUDF(sDOM, 'Address')
					tempVal = curradd
					api.setUDF(sDOM, 'Address', tempVal)

					# Set 'LastSupDate' UDF
					tempDate = api.getUDF(sDOM, 'LastSupDate')
					tempDate = today_date
					api.setUDF(sDOM, 'LastSupDate', tempDate)
					...
					# if there are no suggestions, and overall score is < 80

					if (len(currsupp) == 1) and (currsupp[0] is '') and (overallscore < 80):
	...
	if len(salesDict) > 0:
		...
		for k, v in salesDict.items():
			if v[2] == 99:
				salesYes = salesYes + ' - ' + v[0] + ' ' + v[1] + '(Partner)' + '\r\n'
			else:
				salesYes = salesYes + ' - ' + v[0] + ' ' + v[1] + '\r\n'

		sendMail2(salesYes, 'salesteam@ixcela.com', 'TEST: Sales Kits Results Posted')
	...
	if upgradeDict:
		...
		#upgUrl = 'https://ixcela.com/api/v1/start-trial.json'
...
def main():
	...
	args[ "processLuid" ]= "24-3278"
	args[ "username" ] = "ssarangi"
	#args[ "password" ] = "Ixcela16!"
	args[ "password" ] = "WolfEmoji316"
	#args['stepURI'] = "https://ixcela-test.claritylims.com/api/v2/configuration/protocols/53/steps/104"
	args['stepURI'] = "https://ixcela.claritylims.com/api/v2/configuration/protocols/53/steps/60"
	args['filename'] = 'suppfile'
